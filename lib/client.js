/**
 * dsh-deepseek-capture — DeepSeek 对话捕获插件（客户端半区，Web UI）
 *
 * 在对话区视图 tab（conversation.view 槽位）注册「⛁ DeepSeek 捕获」页，
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── 文案 ──────────────────────────────────────────────────────────────
		const NS = "deepseek-capture";
		const zh = {
			"viewLabel": "DeepSeek 捕获",
			"title": "⛁ DeepSeek 对话捕获",
		...
		const en = {
			"viewLabel": "DeepSeek Capture",
			"title": "⛁ DeepSeek Capture",
		...
		function apply(ctx) {
			ctx.effect(() => ctx.locale.register(NS, { zh, en }), "dsh-deepseek-capture: dictionaries");
