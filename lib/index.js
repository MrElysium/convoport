/**
 * convoport — DeepSeek Harness「Convoport」插件（服务端半区）
 *
 ...
 *
 * 客户端半区（侧边栏「Convoport」面板）见 lib/client.js。
 */
...
const SKILL_CONTENT = [
  ...
  "",
  "### 导入",
  "- 用户可通过侧边栏「Convoport」面板按消息挑选导入；导入 = 新建 Harness 会话，选中消息成为历史。",
...
class CaptureStore {
  ...
  async init() {
    ...
    if (!this.sqliteOk) {
      console.warn("[convoport] node:sqlite unavailable — search disabled (JSON store still works)");
    }
  }
...
async function importIntoSession(ctx, store, { id, indexes, sessionId: currentSessionId }) {
  ...
  if (!cwd && currentSessionId) {
    console.warn(`[convoport] current session ${currentSessionId} has no cwd — imported session may not appear in a workspace`);
  }
  ...
  if (cwd) {
    try {
      ...
      if (workspace) {
        ...
        console.warn(`[convoport] no workspace owns "${cwd}" — imported session may appear under 未分组`);
      }
    } catch (error) {
      console.warn(`[convoport] attach to workspace failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
export function apply(ctx) {
  ...
  ctx.commands.register({
    name: CMD_NAME,
    description: "查看 Convoport统计与资产概览",
    handler: async () => {
      await ensureInit();
      ...
      return {
        ...
        text: [
          ...
          recent || "（暂无）",
          "",
          "在侧边栏「Convoport」面板可按消息挑选并导入到当前工作区新建会话。"
        ].join("\n")
      };
    }
  });
  ...
  ctx.skills.register({
    ...
    source: "runtime",
    metadata: { version: "0.1.1", source: "convoport" }
  });
}
